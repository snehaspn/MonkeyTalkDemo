...
public class MTUrlToolbar extends LinearLayout {
	...
	public MTUrlToolbar(Context context, AttributeSet attrs) {
		...
		goButton().setContentDescription(context.getString(R.string.url_go));

		toolbarHeight = this.getMeasuredHeight();

		// this.urlField().setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT,
		// 70));
	}
	...
	private Button goButton() {
		return (Button) findViewById(R.id.btnGo);
	}
	...
	private OnClickListener goButtonListener() {
		return new OnClickListener() {
			public void onClick(View v) {
				InputMethodManager inputManager = (InputMethodManager) v.getContext()
						.getSystemService(Context.INPUT_METHOD_SERVICE);
				...
				goToUrl(formattedUrl(urlField().getText().toString(), true));

				// urlField().dispatchKeyEvent(KeyEvent.KEYCODE_ENTER);
				// goToUrl(formattedUrl(urlField().getText().toString(), true));
			}
		};
	}
		...
		if (browserActivity().mUrlToolbar.getPaddingTop() != padding) {
			...
			AnimationListener animListener = new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					...
					browserActivity().mUrlToolbar.setPadding(
							browserActivity().mUrlToolbar.getPaddingLeft(), top,
